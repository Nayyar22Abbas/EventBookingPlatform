...
const TimeSlotsPage = () => {
  ...
  if (error && slots.length === 0) return <div className="min-h-screen bg-gradient-to-br from-[#FFF8DC] via-[#F5DEB3] to-[#FFD700] flex items-center justify-center"><p className="text-red-600 text-xl">❌ {error}</p></div>;
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FFF8DC] via-[#F5DEB3] to-[#FFD700] p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <motion.div className="mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-5xl md:text-6xl font-extrabold text-[#7a2222] mb-2">Time <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#bfa544] to-[#ffd700]">Slots</span></h1>
          <p className="text-xl text-gray-700">Manage your hall availability and booking times</p>
        </motion.div>
        {slots.length === 0 ? (
          <motion.div className="text-center py-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="text-6xl mb-4">⏰</div>
            <p className="text-gray-700 text-xl">No time slots added yet</p>
            <motion.button onClick={() => setShowModal(true)} className="mt-4 px-8 py-3 bg-gradient-to-r from-[#bfa544] to-[#8b7a2a] text-[#7a2222] rounded-xl font-bold hover:shadow-lg transition" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>➕ Add Time Slot</motion.button>
          </motion.div>
        ) : (
          <>
            <motion.div className="space-y-4" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }} initial="hidden" animate="visible">
              {slots.map((slot) => (
                <motion.div key={slot._id} className="bg-gradient-to-br bg-white/80 backdrop-blur-xl border border-[#bfa544]/20 rounded-2xl p-6 hover:border-[#bfa544]/50 transition" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} whileHover={{ y: -2 }}>
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                    <div><p className="text-sm text-gray-600 mb-1">Hall</p><p className="text-lg font-bold text-[#7a2222]">{slot.hall?.name}</p></div>
                    <div><p className="text-sm text-gray-600 mb-1">Date</p><p className="text-lg font-bold text-[#7a2222]">{new Date(slot.date).toLocaleDateString()}</p></div>
                    <div><p className="text-sm text-gray-600 mb-1">Start</p><p className="text-lg font-bold text-[#bfa544]">{slot.startTime}</p></div>
                    <div><p className="text-sm text-gray-600 mb-1">End</p><p className="text-lg font-bold text-[#ffd700]">{slot.endTime}</p></div>
            ...
            <motion.div className="mt-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              <motion.button onClick={() => setShowModal(true)} className="w-full py-4 px-6 bg-gradient-to-r from-[#bfa544] to-[#8b7a2a] text-[#7a2222] rounded-xl font-bold border border-[#ffd700]/20 hover:shadow-lg transition" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>➕ Add Another Time Slot</motion.button>
            </motion.div>
          </>
        )}
        ...
        <AnimatePresence>
          {showModal && (
            ...
            >
              ...
              >
                <h2 className="text-3xl font-bold text-[#7a2222] mb-6">Add Time Slot</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 bg-slate-700/50 border border-[#bfa544]/20 rounded-lg text-[#7a2222] focus:outline-none focus:border-[#bfa544]"
                  />
                  <div className="flex gap-3 mt-6 pt-4 border-t border-[#bfa544]/20">
                    <motion.button
                      type="button"
                      onClick={() => {
                        setShowModal(false);
