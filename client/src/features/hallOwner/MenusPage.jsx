...
const MenusPage = () => {
  ...
  if (error && menus.length === 0) return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center"><p className="text-red-400 text-xl">❌ {error}</p></div>;

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <motion.div className="mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <h1 className="text-5xl md:text-6xl font-extrabold text-white mb-2">Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#bfa544] to-[#ffd700]">Menus</span></h1>
          <p className="text-xl text-gray-300">Manage your catering menus and pricing</p>
        </motion.div>
        {menus.length === 0 ? (
          <motion.div className="text-center py-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="text-6xl mb-4">🍽️</div>
            <p className="text-gray-300 text-xl">No menus added yet</p>
            <motion.button onClick={() => setShowModal(true)} className="mt-4 px-8 py-3 bg-gradient-to-r from-[#bfa544] to-[#8b7a2a] text-white rounded-xl font-bold hover:shadow-lg transition" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>➕ Add New Menu</motion.button>
          </motion.div>
        ) : (
          <>
            <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1 } } }} initial="hidden" animate="visible">
              {menus.map((menu) => (
                <motion.div key={menu._id} className="bg-gradient-to-br from-slate-800/50 to-slate-700/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 hover:border-[#bfa544]/50 transition" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} whileHover={{ y: -5 }}>
                  <div className="text-4xl mb-3">🍽️</div>
                  <h3 className="text-xl font-bold text-white mb-2">{menu.name}</h3>
                  <p className="text-sm text-gray-400 mb-3">📍 {menu.hallName}</p>
                  <p className="text-2xl font-bold text-[#bfa544] mb-4">₹{menu.pricePerPlate}/plate</p>
                  <p className="text-sm text-gray-300 mb-4 line-clamp-3">{menu.items.join(', ')}</p>
            ...
            <motion.div className="mt-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
              <motion.button onClick={() => setShowModal(true)} className="w-full py-4 px-6 bg-gradient-to-r from-[#bfa544] to-[#8b7a2a] text-white rounded-xl font-bold border border-[#ffd700]/20 hover:shadow-lg transition" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>➕ Add Another Menu</motion.button>
            </motion.div>
          </>
        )}
        ...
        <AnimatePresence>
          {showModal && (
            ...
            >
              ...
              >
                <h2 className="text-3xl font-bold text-white mb-6">Add New Menu</h2>
